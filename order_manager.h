...
void om_add_new_dropoff_only_order (Floor_t dropoff_floor);	//This has to be implemented
Order_t* om_get_first_order (void);
void om_add_dropoff_to_order (Floor_t pickup_floor, Direction_t direction, Floor_t dropoff_floor); //Direction not adress
void om_remove_order (Order_t* order);
void om_clear_all_orders (void);
Order_t* om_contains_pickup (Floor_t pickup_floor, Direction_t direction); //This returns the order
